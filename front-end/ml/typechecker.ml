 ...
 *
 *    Description:  Typechecker for Ruse ML
 *
...
(* Requires the Ruse specification and Leroy Modular Modules *)
open Ruse
open Modules
...
struct
    module Core = RuseML
    module Env = RuseMLEnv
    open RuseML (* TODO necessary ? *)
    ...
    let expand_manifest env path args =
      match Env.find_type path env with
        {RuseMLMod.manifest = None} ->
          raise Cannot_expand
      | {RuseMLMod.manifest = Some def} ->
    ...
    let rec check_simple_type env params ty =
      ...
      | Typeconstr(path, tl) ->
          let arity = (Env.find_type path env).RuseMLMod.kind.arity in
 ...
 *-----------------------------------------------------------------------------*)
module RuseMLModTyping = Mod_typing(RuseMLMod)(RuseMLEnv)(RuseMLTyping)
