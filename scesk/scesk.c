...
FUNCTIONALITY VALUE evalAtom(VALUE atom){
    ...
    case SI : {
        ...
        // make Continue continuation TODO make function
        KONT kk; 
        kk.c    = MALLOC(sizeof(struct N(KCont)));
        kk.c->t = N(KCONTINUE);
        kk.c->next = mystate->cont;
        kk.c->e = N(copyBinding)(mystate->env);
        mystate->cont = kk;
...
LOCAL LIMBO step()
{
    ...
    switch(mystate->control.b->t){
        ...
        case N(LET) : {
            ...
            struct N(KLet) * nn = MALLOC(sizeof(struct N(KLet)));
            nn->var  = b;
            nn->expr = c;
            nn->e    = mystate->env; 
            nn->next = mystate->cont;
            nn->t = N(KLET);
            KONT new;
            new.l = nn;
            mystate->cont = new;
